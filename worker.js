...
async function aggregateProspectContext(prospectId, authProfile, env) {
  try {
    ...
    const tagsResponse = await fetch(`https://kwebsccgtmntljdrzwet.supabase.co/rest/v1/prospect_tags?prospect_id=eq.${prospectId}&company_id=eq.${clientId}&select=tag,applied_at,tags_taxonomy!inner(definition)`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${env.SUPABASE_SERVICE_KEY}`,
        'apikey': `${env.SUPABASE_SERVICE_KEY}`,
    ...
    const communicationsResponse = await fetch(`https://kwebsccgtmntljdrzwet.supabase.co/rest/v1/communications?lead_id=eq.${prospectId}&company_id=eq.${clientId}&select=type,recipient,content,created_at,external_id&order=created_at.desc&limit=50`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${env.SUPABASE_SERVICE_KEY}`,
        'apikey': `${env.SUPABASE_SERVICE_KEY}`,
...
async function scheduleProspectAction(prospectId, authProfile, env) {
  try {
    ...
    const rateLimitResponse = await fetch(`https://kwebsccgtmntljdrzwet.supabase.co/rest/v1/task_queue?prospect_id=eq.${prospectId}&company_id=eq.${clientId}&status=eq.PENDING&created_at=gte.${new Date(Date.now() - 60 * 60 * 1000).toISOString()}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${env.SUPABASE_SERVICE_KEY}`,
        'apikey': `${env.SUPABASE_SERVICE_KEY}`,
    ...
    const communicationsResponse = await fetch(`https://kwebsccgtmntljdrzwet.supabase.co/rest/v1/communications?lead_id=eq.${prospectId}&company_id=eq.${clientId}&select=consent_status&order=created_at.desc&limit=1`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${env.SUPABASE_SERVICE_KEY}`,
        'apikey': `${env.SUPABASE_SERVICE_KEY}`,
